...
public static class SkillAttackDetectionTool
{
    ...
    public static Collider[] BoxDetection(Transform modelTransform, AttackBoxDetectionData data, LayerMask layerMask)
    {
        ...
        Physics.OverlapBoxNonAlloc(modelTransform.position + data.Position, data.Scale / 2, detectionResluts, modelTransform.rotation * Quaternion.Euler(data.Rotation), layerMask);
    ...
    {
        CleanDetectionResluts();
        Physics.OverlapSphereNonAlloc(modelTransform.position + data.Position, data.Radius, detectionResluts, layerMask);
        return detectionResluts;
    ...
    public static Collider[] FanDetection(Transform modelTransform, AttackFanDetectionData data, LayerMask layerMask)
    {
        ...
        size.z = size.x;
        size.y = data.Height;
        Vector3 fanPosition = modelTransform.position + data.Position;
